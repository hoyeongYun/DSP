...
class ETE_Per_Item_Dataset(torch.utils.data.Dataset):
    '''
    Args:
        df: input data
    ...
    def __init__(self, df: pd.DataFrame, 
        ...
        temp = temp.to_numpy().reshape(-1, self.total_months, self.req_cols)
        self.x = torch.FloatTensor(temp).to(self.device)
        if self.mode == 'train_eval':
            temp = df[self.target_col]
            temp = temp.to_numpy().reshape(-1, self.total_months, 1)
            self.y = torch.FloatTensor(temp).to(self.device)
        else:
            self.y = None
        self.store_ids = torch.FloatTensor(df['Store'].to_numpy().reshape(-1, self.total_months)[:, 0]).to(self.device)
